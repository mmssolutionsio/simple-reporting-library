...
const controlFilePath = join(folders.root, 'migration', 'control.md');
...
function createContext(options) {
  const sourceRoot = normalizePath(
    resolveProjectPath(options.source ?? 'migration'),
  );
  ...
  return {
    ...
    mappingsPath: firstExisting([
      ...
      join(folders.root, 'mapping.json'),
      join(folders.root, 'mappings.json'),
    ]),
  ...
  const components = migratableComponents.map(
    ({ component, mapped, name, source, html, ldConf }, index) => {
      ...
      return {
        ...
        order,
        targetFolder: join(folders.ld, deprecatedGroup, `${order}.${name}`),
        html,
...
function collectExistingComponents() {
  ...
  collectExistingComponentsInFolder(folders.ld, components);
  return components;
}
...
function collectExistingComponentsInFolder(folder, components) {
  ...
  for (const entry of readdirSync(folder, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (folder === folders.ld && entry.name === deprecatedGroup) continue;
    ...
    try {
      ...
      console.warn(
        `Could not parse existing ${relative(folders.root, path)}: ${error.message}`,
      );
    }
  }
}
...
function normalizeScss(context, mappings) {
  ...
  for (const file of globalFiles) {
    ...
    if (existsSync(source)) {
      scss.push({
        source,
        target: join(folders.srlAssets, 'scss', basename(file)),
      });
    }
  }
...
function writeProperties(context, properties) {
  for (const [name, property] of Object.entries(properties)) {
    const target = join(folders.ld, '999.Properties', name, 'properties.json');
...
function writeControlFile(context, plan) {
  ...
  console.log(`Write ${relative(folders.root, controlFilePath)}`);
}
...
function createControlFileContent(context, plan) {
  const lines = [
    ...
    '',
    `Migration source: \`${relative(folders.root, context.sourceRoot) || '.'}\``,
    `Legacy target: \`${relative(folders.root, join(folders.ld, deprecatedGroup))}\``,
  ...
  if (plan.components.length === 0) {
    ...
    lines.push(
      '| Order | Name | Target | Source | SCSS files |',
      '| --- | --- | --- | --- | --- |',
    );
    ...
    for (const component of plan.components) {
      lines.push(
        `| ${[
          component.order,
          `\`${component.name}\``,
          `\`${relative(folders.root, component.targetFolder)}\``,
          component.source
            ? `\`${relative(folders.root, component.source)}\``
            : '**placeholder**',
...
function mergeLivingdocsConfig(plan) {
  const target = join(folders.root, 'livingdocs.config.json');
...
function writeTextFile(context, target, content) {
  if (existsSync(target) && !context.force) {
    console.log(
      `Skip existing ${relative(folders.root, target)} (use --force to overwrite)`,
    );
    return;
  }
  mkdirSync(join(target, '..'), { recursive: true });
  writeFileSync(target, content);
  console.log(`Write ${relative(folders.root, target)}`);
}
...
function resolveProjectPath(path) {
  return isAbsolute(path) ? path : join(folders.root, path);
}
