...
import { statSync, readFileSync, writeFileSync, cpSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { createRequire } from 'node:module';
import { baseComponentsToInstall } from './config.js';
...
async function init(folder, options) {
  ...
  try {
    ...
    await fs.copy(join(__dirname, '..', 'dev'), projectPath).then(async () => {
      ...
      baseComponentsToInstall.forEach((component) => {
        cpSync(
          join(__dirname, '..', 'livingdocs', ...component.split('/')),
          join(projectPath, 'livingdocs', ...component.split('/')),
